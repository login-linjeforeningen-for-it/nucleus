...
export async function getSafeMusicActivity(): Promise<NativeMusicActivity> {
    ...
    return {
        ...
        currentlyListening: Array.isArray(data.currentlyListening)
            ...
            : [],
        topFiveToday: Array.isArray(data.topFiveToday) ? data.topFiveToday.map((entry: any) => normalizeMusicRow(entry)) : [],
        topFiveThisWeek: Array.isArray(data.topFiveThisWeek) ? data.topFiveThisWeek.map((entry: any) => normalizeMusicRow(entry)) : [],
        topFiveThisMonth: Array.isArray(data.topFiveThisMonth) ? data.topFiveThisMonth.map((entry: any) => normalizeMusicRow(entry)) : [],
    }
}
