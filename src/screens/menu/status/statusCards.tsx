...
export function ServiceDetails({ service, onEdit }: { service: NativeMonitoringService, onEdit: () => void }) {
    const { theme } = useSelector((state: ReduxState) => state.theme)
                ...
                {service.bars.slice(0, 8).map((bar, index) => (
                    <View key={`${service.id}-${bar.timestamp}-${index}`} style={{
                        ...
                        <Text style={{ ...T.text12, color: bar.status ? '#7ee787' : '#ff8b8b' }}>
                            {bar.status ? 'UP' : bar.expectedDown ? 'MAINTENANCE' : 'DOWN'} · {Math.round(bar.delay || 0)} ms
                        </Text>
