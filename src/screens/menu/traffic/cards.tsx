    ...
    return (
        <Cluster style={{
            ...
            <View style={{ padding: 12 }}>
                ...
                <Space height={10} />
                {entries.slice(0, 6).map(entry => <MetricRow key={entry.key} entry={entry} total={total} timeValue={timeValue} />)}
...
function MetricRow({ entry, total, timeValue }: { entry: TrafficEntry, total: number, timeValue?: boolean }) {
    const { theme } = useSelector((state: ReduxState) => state.theme)
    ...
    return (
        <View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 8 }}>
                <Text style={{ ...T.text12, color: theme.textColor, flex: 1 }} numberOfLines={1}>
                    {entry.key}
                </Text>
            ...
            <View style={{ height: 6, borderRadius: 999, backgroundColor: '#ffffff12', overflow: 'hidden' }}>
                <View style={{ width: `${width}%`, height: 6, borderRadius: 999, backgroundColor: theme.orange }} />
            </View>
            <Space height={10} />
        </View>
    )
