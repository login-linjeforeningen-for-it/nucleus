    ...
    switch (event.organization?.shortname) {
        ...
        default:
            return event.organization
                ? (lang ? event.organization.name_no || event.organization.name_en : event.organization.name_en || event.organization.name_no)
                : (lang ? event.category.name_no || event.category.name_en : event.category.name_en || event.category.name_no)
    }
}
...
function getMazemapUrl(event: GetEventProps) {
    ...
    return `https://use.mazemap.com/#v=1&campusid=${location.mazemap_campus_id}&sharepoitype=poi&sharepoi=${location.mazemap_poi_id}`
}
        ...
        <>
            ...
            <SectionCard title={lang ? "Oversikt" : "Overview"}>
                <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10 }}>
                    <MetaChip label={lang ? "Kategori" : "Category"} value={lang ? event.category.name_no || event.category.name_en : event.category.name_en || event.category.name_no} />
                    <MetaChip label={lang ? "Arrangør" : "Organizer"} value={getOrganizerName(event, lang)} />
                    <MetaChip label={lang ? "Sted" : "Location"} value={location} />
                    <MetaChip label={lang ? "Plasser" : "Capacity"} value={formatCapacity(event, lang)} />
                    <MetaChip label={lang ? "Publikum" : "Audience"} value={audience} />
                    <MetaChip label={lang ? "Format" : "Format"} value={event.digital ? (lang ? "Digitalt" : "Digital") : (lang ? "Fysisk" : "In person")} />
                </View>
            </SectionCard>
            {shortInfo ? (
                <>
